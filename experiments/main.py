    ...
    with torch.no_grad():
        ...
        if args.dataset == 'custom':
            ...
            [
                ...
                (torch.tensor(np.power(2, list(range(5)) * 10000).reshape(-1, 5)).to('cuda') * model(x.to('cuda').round())).to(torch.float32).sum().item() - sum(y.to('cuda') * torch.tensor(np.power(2, list(range(5)) * 10000).reshape(-1, 5)).to('cuda')).to(torch.float32).sum().item()
            for x, y in loader
            ])
        elif args.dataset == 'vectormul':
            res = np.mean(
