    ...
    with torch.no_grad():
        ...
        if args.dataset == 'custom':
            ...
            res = torch.tensor([
                #(model(x.to('cuda').round()).argmax(-1) == y.to('cuda')).to(torch.float32).mean().item()
                #((model(x.to('cuda').round()) == y.to('cuda')).to(torch.float32)).sum().item() / 500
                ((torch.tensor(np.power(2, list(range(5)) * x.size()[0]).reshape(-1, 5)) * model(x.to('cuda')).round().to(torch.float32).cpu()) - (y.cpu() * torch.tensor(np.power(2, list(range(5)) * x.size()[0]).reshape(-1, 5)).to(torch.float32))).mean(dim=0).numpy()
            for x, y in loader
            ]).mean(dim=0)
        elif args.dataset == 'vectormul':
            res = np.mean(
