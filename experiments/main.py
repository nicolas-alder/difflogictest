    ...
    with torch.no_grad():
        ...
        if args.dataset == 'custom':
            ...
            [
                ...
                (torch.tensor(np.power(2, list(range(5)) * 100).reshape(-1, 5)) * model(x.to('cuda')).round().to(torch.float32)) - (y.to('cuda') * torch.tensor(np.power(2, list(range(5)) * 100).reshape(-1, 5)).to(torch.float32))
            for x, y in loader
            ])
        elif args.dataset == 'vectormul':
            res = np.mean(
